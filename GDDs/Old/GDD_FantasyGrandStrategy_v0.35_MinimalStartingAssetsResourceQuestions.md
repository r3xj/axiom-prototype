...
**Version 0.35 — Minimal Starting Assets and Resource Questions Added**
...
| **Wounded / Recovering** | The squad survived but has reduced effectiveness and should return to Hearthmere or a safe support location to recover. Optional if this adds too much complexity. |
...
Equipment Lost or Damaged: optional
...
### 13.26 Prototype Resource and Starting Asset Model
...
#### Resource Model Questions This Section Locks

The following answers should guide v0.1 until playtesting proves otherwise:

1. **Money / liquid value is not required for the first build unless it has a clear use.**  
   Coin, treasury, barter value, stored wealth, credit, tribute, and trade value are important long-term systems, but if v0.1 has no real buying, hiring, trading, wages, or market decisions, money should either be omitted or start at zero.

2. **Labor comes from settlement population.**  
   Hearthmere has a population pool. Some portion is available as usable labor/manpower. Labor can be assigned to projects, facilities, surveying, construction, or military service.

3. **Military manpower is labor removed from the civilian pool.**  
   Creating a squad pulls manpower out of Hearthmere’s available labor. Dissolving a squad at Hearthmere returns surviving manpower to the pool. Soldiers killed in combat should not return; they reduce available population/manpower until a future population-growth system exists.

4. **Prototype labor should use simple assignment units, not a full population simulation.**  
   The first build can model labor as “labor teams” or “commitment capacity” rather than exact individual workers. A mine, forge project, academy analysis, survey, guard post, or squad can each consume one or more commitment units.

5. **Supplies are concrete goods produced over time.**  
   Supplies are produced by settlements, stored locally, shipped physically, and consumed by remote work and military activity. Supplies are the basic in-world limiter on expansion, industry, and field operations.

6. **Armory stockpiles can start at zero.**  
   The player does not need to start with spare weapons and armor waiting in storage. If there are no starting field squads, stockpiled basic weapons and armor can begin at zero. The settlement may still have an abstract home defense value so Hearthmere is not defenseless before the first squad is created.

7. **Starting squads can start at zero.**  
   The prototype does not need a Civ-style starting scout. The player starts with known nearby opportunities and chooses whether to create a squad, assign survey labor, build security, or begin prospecting.

8. **Prospect opportunities come from regions, not only obvious hill icons.**  
   Hills, foothills, riverbeds, forests, ruins, wetlands, caves, old quarries, magical anomalies, and settlement rumors can all generate prospect leads. For v0.1, each region can contain one or more surveyable prospect opportunities, some fruitful and some not.
...
|---|---|
| **Population / Labor Pool** | Provides workers/manpower for surveying, mining, forging, research, guard duty, squad creation, and basic settlement tasks. Represented abstractly. |
| **Supplies** | Concrete physical goods produced by settlements over time and consumed by mines, remote squads, caravans, guard posts, and eventually settlements themselves. Supplies exist in local stockpiles and must be shipped where needed. |
...
| **Squads / Parties** | Physical units created from manpower and equipment. They can move directly or be assigned to guard, escort, scout, survey, patrol, or attack. |
| **Facilities** | Hearthmere, Forge, Academy, Stockpile, and any established Mine/Extraction Site. |
| **Coin / Liquid Value** | Optional for v0.1. Include only if the prototype has a concrete action that spends or generates it. Otherwise omit or start at zero. |
| **Basic Weapons / Armor Stockpiles** | Optional starting stockpiles. For the current v0.1 direction, spare stockpiles should start at zero unless needed for immediate squad creation testing. |

#### Minimal Starting Assets for v0.1

The current prototype direction favors a lean start:

- **Hearthmere**, the capital settlement.
- **Basic knowledge of nearby regions**, including major routes, several prospect opportunities, and at least one known danger area.
- **Population / labor pool**, available for assignment.
- **Hearthmere supply production** and a small local supply stockpile.
- **Forge and Academy access**, likely idle at start until labor/materials are assigned.
- **Several visible prospect leads**, distributed across nearby regions.
- **No starting field squads**, unless testing shows the opening needs one.
- **No spare armory stockpile**, unless testing shows squad creation is blocked too harshly.
- **No starting coin/liquid value**, unless a spending loop is implemented.

This means the first few minutes are not empty, because the player still has decisions:

- survey which prospect first,
- allocate labor to survey, production, forge preparation, or academy readiness,
- create a squad or delay military investment,
- begin supply production/stockpiling,
- inspect known routes and danger areas,
- and decide whether the opening posture is exploration, security, or infrastructure.

The player starts with **capacity**, not finished answers.

#### Settlement Defense vs. Field Squads

If starting squads and armory stockpiles begin at zero, Hearthmere still needs a minimal abstract defense so the first raider event does not instantly end the scenario.

For v0.1, distinguish:

- **Settlement Defense** — an abstract local defense value representing walls, town watch, able-bodied residents, and improvised defense. It protects Hearthmere but cannot leave the settlement.
- **Field Squads** — mobile units created by assigning manpower and equipment. They can scout, escort, guard remote sites, patrol, attack, or establish guard posts.

This lets the player begin without a mobile army while still making the decision to create one meaningful.

#### Labor and Manpower

Labor is the player’s most important early constraint. The prototype should represent it simply, but consistently.

Possible v0.1 model:

- Hearthmere has a fixed number of **available labor teams**.
- Each active project or facility consumes one or more teams.
- Creating a field squad consumes manpower/labor capacity.
- Dissolving a squad at Hearthmere returns surviving manpower.
- Wounded squads temporarily tie up manpower until recovery.
- Killed soldiers permanently reduce the available manpower pool for the scenario.

This does not need a full population-growth, class, tax, or family simulation. It only needs to make the player feel that every assignment has an opportunity cost.
...
- **Supplies are stored locally.** Hearthmere begins with a small supply stockpile. A mine, caravan, squad, or remote guard post only has the supplies physically present there or delivered there.
...
The starting economy should be tuned around meaningful commitments rather than precise final numbers. The player should begin with enough labor, supplies, and settlement capacity to pursue several important actions, but not enough to pursue every possible priority at full efficiency.
...
- **Survey + Prepare Forge + Stockpile Supplies**: search for opportunity while preparing to exploit it.
- **Survey + Create Squad + Scout Danger Area**: prioritize information and security before industry.
- **Mine + Research + Escort**: extract ore, study it, and protect shipments.
- **Mine + Forge + Defend**: exploit the ore, make equipment, and keep enough security at home.
...
For the current v0.1 direction, spare armory stockpiles can start at zero:

- **Basic Weapons** are required for normal mobile squad attack effectiveness.
- **Basic Armor / Shields** are required for normal mobile squad defense effectiveness.
- **Improvised arms** may allow an emergency squad, but it should be weak.
...
Starting at zero makes the early armory decision meaningful. The player must choose whether to produce basic equipment, rush a mobile squad with poor gear, or delay military projection until Redglass equipment is available.
...
For v0.1, Value / Appeal should only have an effect if there is a system that uses it. Possible lightweight uses include:

- higher raider interest,
- higher claim pressure,
- higher reward if delivered to a settlement/project,
- or higher sale/trade value if a simple trade action exists.

If v0.1 has no trade or sale loop, Value / Appeal can still exist as a revealed property but should not pretend to be strategically important yet.
...
### 13.27 Updated v0.1 Build Scope
...
**Starting assets:** The player starts with Hearthmere, basic knowledge of nearby regions, several visible prospect leads, a population/labor pool, Hearthmere supply production, a small local supply stockpile, and basic Forge/Academy access. Current v0.1 direction: starting field squads = 0, spare armory stockpiles = 0, and coin/liquid value = 0 or omitted unless a spending loop is implemented. Hearthmere may have an abstract local defense value so the capital is not defenseless, but mobile military power must be created by assigning manpower and equipment.
...
**The test:** One settlement, multiple prospect sites, one strange ore deposit discovered through survey, one mine that consumes supplies, at least one supply shipment, two route choices, one forge/workshop, one academy or research action, one equipment fork, one flexible militia/guard squad with manpower, supply use, movement, assignments, and equipment coverage, one local raider actor capable of contesting the ore chain, and scenario endings that test whether discovery can become strategy. If the player feels curiosity, pressure, and adaptation from that scenario, the foundation works.

### 13.28 Next Design Questions

The next unresolved prototype questions are:

1. What exact v0.1 labor model should be used: abstract commitment slots, labor teams, or numeric population/manpower?
2. What actions, if any, should use coin/liquid value in the first build? If there are none, money should remain omitted or start at zero.
3. How should the player create the first mobile squad when armory stockpiles start at zero: basic equipment project, improvised weak squad, or Redglass-first equipment?
4. What information appears in the Material Codex after each discovery step for one example Redglass roll?
5. What minimum combat-result UI is needed to show outcome, losses, cargo status, equipment performance, supply impact, and field discoveries?
6. Which prototype victory paths should be required for the first playable build versus treated as optional stretch goals?

*Recommended next session: define the v0.1 labor/manpower model and first squad creation flow, then define the concrete combat result UI and one example Redglass Codex reveal sequence.*
