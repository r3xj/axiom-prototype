...
**Version 0.63 — Tech Stack and Prototype Platform Recommendation Added**
...
### 13.53 Phase 1A Solo Developer Task Tracker

This task tracker converts the solo-developer execution plan into a practical build sequence. It is intentionally small, linear, and milestone-driven. The goal is not to build AXIOM correctly forever. The goal is to build the smallest playable systems demo that proves the non-combat discovery/logistics loop.

Phase 1A should be treated like a graybox prototype. Use placeholder art, simple panels, obvious buttons, hardcoded data, and debug tools. Do not build procedural generation, polished UI, raider AI, tactical combat, or deep economy yet.

Core rule:

> If a task does not help the player inspect a known area, survey a prospect, establish a mine, move physical goods, discover a material property, or complete one meaningful output, it does not belong in Phase 1A.

#### Phase 1A Completion Target

Phase 1A is complete when the player can do this from a fresh scenario start:

```text
Inspect Hearthmere
Inspect prospect sites
Survey prospect
Confirm useful deposit
Claim deposit
Establish mine
Ship supplies to mine
Produce ore at mine
Ship ore to Hearthmere
Run material test / analysis
Update Codex
Complete one meaningful output
See summary screen
Reset and replay with a different material variant
```

No raiders are required. No combat is required. No AI is required. No procedural map generation is required.

#### Milestone 0 — Project Skeleton

Goal: create a runnable prototype shell.

Tasks:

- [ ] Choose engine/tooling.
- [ ] Create project repository.
- [ ] Add simple scene/screen for the strategic map.
- [ ] Add placeholder UI shell: top bar, map area, selected-object panel, notification/event log.
- [ ] Add pause/play state.
- [ ] Add current in-game hour/day display.
- [ ] Add basic save/reset only if cheap; otherwise rely on scenario reset.

Acceptance check:

- The project launches.
- The player sees a placeholder strategic map and can pause/play time.
- There is a visible place where object details will appear.

Cut line:

- Do not build settings menus, main menu polish, camera polish, art pipeline, animation, audio, or save/load complexity yet.

#### Milestone 1 — Hardcoded Scenario Map

Goal: create the smallest inspectable world.

Tasks:

- [ ] Add hardcoded regions: Hearthmere, Redglass Foothills, Ashen Pass, Old Pine Road, Westmere Farms.
- [ ] Optional: add Blackbanner Camp and The Silent Border as nonfunctional flavor nodes, but do not implement raiders.
- [ ] Add route connections between regions.
- [ ] Make regions clickable.
- [ ] Show selected region details in the selected-object panel.
- [ ] Show connected routes for selected region.
- [ ] Add simple map labels/icons.

Acceptance check:

- The player can click each region and route.
- The player understands where Hearthmere, prospect areas, and routes are.
- The map already implies physical movement even before movement exists.

Cut line:

- Do not build procedural map generation.
- Do not build fog of war.
- Do not build pathfinding beyond hardcoded route traversal.
- Do not build route danger or raiders yet.

#### Milestone 2 — Hearthmere Economy Foundation

Goal: make Hearthmere a real source of population, labor, supplies, and local storage.

Tasks:

- [ ] Add Hearthmere settlement data.
- [ ] Add population fields: total population, available workforce, labor teams.
- [ ] Add local Hearthmere stockpiles.
- [ ] Add supplies as a physical local good.
- [ ] Add daily supply production if a labor team is assigned.
- [ ] Add labor assignment data.
- [ ] Add simple Hearthmere panel showing population, labor teams, supplies, and active assignments.
- [ ] Add ability to assign/unassign labor team to supply production.
- [ ] Add hourly tick that can advance time and trigger daily supply production.

Acceptance check:

- Hearthmere produces supplies over time when labor is assigned.
- Labor assignment visibly reduces available labor capacity.
- Supplies are shown as stored at Hearthmere, not as a global number.

Cut line:

- Do not build taxes, money, happiness, housing, food variety, wages, professions, specialists, or migration yet.
- Do not build full population growth yet.

#### Milestone 3 — Generic Project System

Goal: create one reusable time-based project system before building prospecting, mining, research, or forging.

Tasks:

- [ ] Add Project object/data structure.
- [ ] Project fields: id, type, location, duration, progress, required labor teams, required inputs, outputs, status.
- [ ] Add active project list.
- [ ] Add hourly project progress if requirements are met.
- [ ] Add project paused/blocked state if requirements are missing.
- [ ] Add project completion event.
- [ ] Add selected-object panel display for active projects.
- [ ] Add debug button to complete selected project.

Acceptance check:

- A dummy project can be started, progress over time, pause if requirements are missing, and complete.
- Project completion can create an event/notification.

Cut line:

- Do not make separate custom systems for every project type yet.
- Survey, mine setup, testing, analysis, and forging should all use this project framework if possible.

#### Milestone 4 — Prospect Sites and Surveying

Goal: make discovery begin as uncertain opportunity.

Tasks:

- [ ] Add three hardcoded prospect sites.
- [ ] Attach prospect sites to regions.
- [ ] Make prospect sites clickable.
- [ ] Add prospect panel with visible clue and unknown outcome.
- [ ] Add Survey Prospect action.
- [ ] Survey creates a project using the generic project system.
- [ ] Add prospect outcomes: true deposit, mundane/low-yield site, false/exhausted site.
- [ ] On survey completion, update prospect state and notify player.
- [ ] Ensure only one site reveals the main useful material in the first test scenario.

Acceptance check:

- The player can inspect multiple prospects.
- At least one prospect is not useful.
- Surveying costs time/labor and reveals an outcome.
- The player experiences discovery before extraction.

Cut line:

- Do not build geology simulation.
- Do not build procedural prospect generation.
- Do not build prospect rarity tables yet.

#### Milestone 5 — Material Profile and First Codex Entry

Goal: create the first material knowledge object.

Tasks:

- [ ] Add MaterialProfile object for one prototype material family.
- [ ] Add 2 hardcoded material variants at minimum: defensive/fire-hardened and offensive/keen-edge.
- [ ] Add random or debug-select material variant at scenario start.
- [ ] Add MaterialDiscoveryState / CodexEntry object.
- [ ] When true deposit is discovered, create first Codex entry.
- [ ] Codex initially shows only minimal observed information.
- [ ] Add Codex panel with material name, source, known properties, unknowns, and field/test notes.
- [ ] Add debug button to reveal next property.

Acceptance check:

- The same prospect clue can create different hidden material variants.
- The player sees only minimal initial information.
- The Codex exists and can update over time.

Cut line:

- Do not build full slider/tag material generation yet.
- Do not build alloys.
- Do not build advanced natural-language generation.

#### Milestone 6 — Deposit Claim and Mine Setup

Goal: turn a discovered deposit into a physical extraction site.

Tasks:

- [ ] Add Deposit/Mine object.
- [ ] Add Claim Deposit action after true prospect is confirmed.
- [ ] Claim Deposit creates a project.
- [ ] Add Establish Mine action after claim completes.
- [ ] Establish Mine creates a project.
- [ ] Mine has local stockpiles: supplies and ore.
- [ ] Mine has status: unclaimed, claimed, establishing, active, paused, blocked.
- [ ] Active mine consumes supplies over time.
- [ ] Active supplied mine produces ore over time.
- [ ] If supplies run out, mine pauses or becomes blocked.

Acceptance check:

- The mine is a place with its own stockpiles.
- Ore appears at the mine, not globally.
- The mine requires supplies to operate.

Cut line:

- Do not build worker animations, mine upgrades, mine damage, guards, raids, or depletion yet.

#### Milestone 7 — Local Stockpiles and No Global Inventory

Goal: enforce the most important logistics rule.

Tasks:

- [ ] Add Stockpile object per location.
- [ ] Hearthmere has stockpile.
- [ ] Mine has stockpile.
- [ ] Stockpiles track supplies, ore, and future outputs.
- [ ] UI always labels stockpile location.
- [ ] Forge/research actions can only use materials present at Hearthmere.
- [ ] Mine operations can only use supplies present at the mine.
- [ ] Add debug display for all local stockpiles.

Acceptance check:

- The player cannot use ore at Hearthmere until it is shipped there.
- The player cannot supply the mine from Hearthmere without a shipment.
- The UI makes local storage obvious.

Cut line:

- Do not add global inventory fallback for convenience.
- Do not hide location-specific goods behind a universal resource bar.

#### Milestone 8 — Shipment and Route System

Goal: make physical goods move.

Tasks:

- [ ] Add Shipment/Caravan object.
- [ ] Shipment fields: cargo, amount, origin, destination, route, progress, status.
- [ ] Add Create Shipment action from Hearthmere to mine for supplies.
- [ ] Add Create Shipment action from mine to Hearthmere for ore.
- [ ] Shipments move over hourly ticks based on route travel time.
- [ ] Shipment cargo is removed from origin stockpile at departure.
- [ ] Shipment cargo is added to destination stockpile on arrival.
- [ ] Shipment is visible/clickable while moving.
- [ ] Shipment panel shows cargo, origin, destination, route, ETA, status.
- [ ] Add event notification on shipment departure and arrival.

Acceptance check:

- Supplies can be shipped to the mine.
- Ore can be shipped back to Hearthmere.
- Goods are unavailable while in transit.
- The player can watch/inspect a shipment moving through the world.

Cut line:

- Do not build raider ambushes yet.
- Do not build escorts yet unless needed for later scaffolding.
- Do not build advanced route choice beyond two hardcoded paths.

#### Milestone 9 — Testing / Academy Analysis

Goal: let knowledge advance through a time-based project.

Tasks:

- [ ] Add Basic Material Test action at Hearthmere.
- [ ] Test requires material sample/ore at Hearthmere.
- [ ] Test creates project.
- [ ] On completion, reveal one or more practical properties in Codex.
- [ ] Add Academy Analysis action if not merged with testing.
- [ ] Analysis creates project and reveals deeper property if completed.
- [ ] Codex update notification links to Codex entry.
- [ ] Properties should be explicit for prototype clarity.

Acceptance check:

- Testing changes what the player knows.
- The Codex becomes more useful after test/analysis.
- The player can understand why testing before forging might matter.

Cut line:

- Do not build targeted experimental tests yet.
- Do not build scholar specialists.
- Do not build multi-property research trees.

#### Milestone 10 — Forge Output / Meaningful Material Output

Goal: turn material into a concrete payoff.

Tasks:

- [ ] Add Forge Output project type.
- [ ] Add output options: weapons, armor/shields, or non-combat material output if needed.
- [ ] Forge project requires ore at Hearthmere.
- [ ] Forge project requires labor/time.
- [ ] Output creates EquipmentBatch or MaterialOutput object at Hearthmere.
- [ ] Output references the material variant and known properties.
- [ ] Output summary explains what is known, unknown, and why it may be useful.
- [ ] If a property is discovered before forging, show it in expected output text.
- [ ] If a property is unknown, indicate uncertainty plainly.

Acceptance check:

- The player can complete one meaningful output from the discovered material.
- The output feels tied to the material’s properties, not generic crafting.
- Phase 1A can end after the output is complete.

Cut line:

- Do not build equipment assignment to squads yet unless Phase 2 has started.
- Do not build detailed recipes, item variants, durability, or equipment inventory UI.

#### Milestone 11 — Phase 1A Summary Screen

Goal: make the prototype loop feel complete.

Tasks:

- [ ] Add Phase 1A completion trigger.
- [ ] Trigger when one meaningful material output is completed.
- [ ] Summary displays: prospects surveyed, deposit found, supplies moved, ore moved, properties discovered, output completed, remaining unknowns.
- [ ] Summary asks whether the player wants to reset/replay.
- [ ] Add reset scenario button.
- [ ] Add replay with different material variant option if cheap.

Acceptance check:

- The player understands what they accomplished.
- The summary reinforces discovery/logistics/economy, not combat.
- The player can replay the loop quickly.

Cut line:

- Do not build full victory/defeat presentation yet.
- Do not build campaign progression.

#### Milestone 12 — Debug and Tuning Pass

Goal: make iteration fast enough for a solo developer.

Tasks:

- [ ] Add debug panel if not already present.
- [ ] Advance time by 1 hour.
- [ ] Advance time by 12 hours.
- [ ] Complete selected project.
- [ ] Add/remove labor team.
- [ ] Add supplies to Hearthmere.
- [ ] Add supplies to mine.
- [ ] Add ore to mine.
- [ ] Add ore to Hearthmere.
- [ ] Reveal next Codex property.
- [ ] Switch material variant.
- [ ] Reset scenario.
- [ ] Print/export current scenario state if cheap.

Acceptance check:

- The developer can test the whole loop quickly.
- The developer can skip waiting without changing code.
- The developer can test different material variants without rebuilding data.

Cut line:

- Do not rely on manual code edits to test normal tuning cases.

#### First End-to-End Build Definition

The first end-to-end build is not finished when all UI is pretty. It is finished when this run works:
...
Forge output.
Read summary.
Reset with different material variant.
```

If this sequence works, Phase 1A exists.

#### Phase 1A Do-Not-Build-Yet List
...
- tactical battles,
- procedural maps,
- procedural material generation beyond fixed variants,
- trade,
- money,
- specialists,
- population growth,
- morale,
- buildings beyond Hearthmere/mine/forge/academy actions,
- upgrades,
- route danger,
- stealth,
- diplomacy,
- gods,
- Legacy Empire activity,
- polished art,
- save/load complexity,
- audio,
- animations.

The solo-developer danger is not that Phase 1A is too small. The danger is that Phase 1A quietly becomes Phase 1–3 before the core loop has been tested.

#### Recommended First Working Day Goal

The first implementation day should not try to build economy or materials. It should aim for this:

```text
A clickable hardcoded map with Hearthmere and three prospect sites.
Clicking an object shows its name, type, and a few placeholder actions.
The clock can advance by one hour.
```

That is enough to prove the project shell and interaction model. Everything else builds from there.

#### Recommended First Week Goal

The first week goal should be:

```text
Clickable map
Hearthmere panel
Labor/supply display
Generic project system
Prospect survey project
One prospect reveals a material and creates a Codex entry
```

If that exists, the prototype has a spine.

#### Recommended First Month Goal

The first month goal should be:

```text
Full Phase 1A loop playable with placeholder UI:
prospect → mine → supply → ore shipment → test → output → summary
```

Do not commit to this as a promise or schedule. It is only a scope compass: if development time is limited, cut everything that does not contribute to this loop.

#### Phase 1A Solo Developer Principle

> Build one complete boring version before building one exciting incomplete version. The boring version teaches what the exciting version must threaten, reward, and complicate.

### 13.54 Tech Stack and Prototype Platform Recommendation

This section translates the solo-developer execution plan into a practical technology recommendation. The purpose is not to pick the most powerful engine. The purpose is to pick the tool that gives one developer the best chance of building Phase 1A without drowning in engine architecture, rendering, tooling, licensing anxiety, or premature polish.

#### Recommendation

Use **Godot 4.x stable**, **GDScript**, and a **2D desktop-first prototype**.
...
Language: GDScript
Presentation: 2D / UI-heavy strategic map
Target platform: Windows desktop first
Data: local JSON or Godot Resources
Persistence: optional simple local save later; not required for first Phase 1A loop
Version control: Git from day one
```

The prototype should be built as a game-like simulation UI, not as a 3D world, not as a polished tactical RPG, and not as a full grand-strategy engine. The first target is a clickable region-node map, local panels, an hourly clock, time-based projects, local stockpiles, shipments, and Codex updates.

Design principle:

> Choose the stack that makes the first complete loop easiest to build, debug, and throw away. Do not choose the stack based on the final dream version of the game.

#### Why Godot Fits AXIOM Phase 1A

Godot is a good fit because Phase 1A is mostly:
...
- and simple movement along routes.

That is well within Godot's strengths. It does not require high-end 3D rendering, networking, advanced physics, or a large marketplace ecosystem.

Godot also keeps the solo-developer workflow simple. A small Godot project can have a single scene, a few panels, some autoloaded systems, and hardcoded data while still being structured enough to grow into later phases.

Useful Godot traits for this prototype:

- **Free/open-source engine** with no per-seat cost for a solo prototype.
- **Strong 2D support** for map icons, routes, UI panels, and overlays.
- **Simple scripting** through GDScript.
- **Fast iteration** for clickable prototypes.
- **Good UI system** for the many inspector panels AXIOM needs.
- **Desktop export path** without needing a web app, app store, or server.

#### Why Not Start With Unity

Unity is powerful and viable, especially for long-term 2D/3D games, but it is probably not the best first choice for this specific solo prototype.

Reasons to avoid Unity for Phase 1A unless there is a strong personal preference:

- more engine/editor complexity than this first prototype needs,
- more temptation to overbuild visuals, prefabs, animations, and production architecture,
- more licensing/business-model baggage to think about,
- and less immediate benefit for a UI-heavy region-node simulation.

Unity remains a reasonable fallback if Godot hits a hard blocker later, but AXIOM Phase 1A does not need Unity's scale.

#### Why Not Start With Bevy / Rust

Bevy and Rust are attractive for simulation-heavy games, but they are a poor fit for a first solo prototype with limited game-development experience.

Reasons to avoid Bevy for Phase 1A:

- Rust has a steeper learning curve,
- ECS architecture can become an additional design problem,
- UI workflow is likely to be more difficult for this kind of panel-heavy prototype,
- and the project needs rapid iteration more than engine elegance.

Bevy might become interesting later if the project turns into a programmer-driven simulation project, but it is not the safest starting point.

#### Why Not Start With a React/Web Prototype

A web prototype is the strongest alternative because the user already has TypeScript/React experience from other projects. A React + TypeScript + Vite prototype could validate the Phase 1A logic quickly, especially because Phase 1A is UI-heavy and has no combat, animation, or pathfinding complexity.

However, the recommended path is still Godot if the intent is to build toward an actual game rather than a design simulator.

Use React/TypeScript only if:

- Godot friction prevents progress,
- the immediate goal is purely to test economy/logistics design,
- or the prototype is meant to become a throwaway design tool rather than the seed of the game.

If a web prototype is used, it should be treated explicitly as disposable. Do not let a React spreadsheet-simulator accidentally become the game's permanent architecture unless that is a deliberate pivot.

#### Desktop First, Not Mobile or Web First

The first build should target **desktop**.

Reasons:

- AXIOM needs information density.
- The player will inspect many panels, routes, stockpiles, projects, and Codex entries.
- Mouse input is natural for a region-node strategy map.
- Desktop avoids mobile layout constraints.
- Web export and mobile builds can wait.

Do not spend Phase 1A time on responsive UI, controller support, touch support, or web deployment. A local desktop prototype is enough.

#### Recommended Godot Project Shape

The first Godot implementation should be intentionally plain.

Recommended main scenes:

```text
Main.tscn
  Root scene containing the game shell.

MapView.tscn
  Displays regions, routes, prospect sites, shipments, and clickable objects.

TopBar.tscn
  Shows time controls, current day/hour, and high-level Hearthmere summary.

InspectorPanel.tscn
  Reusable right-side panel that changes based on selected object.

CodexPanel.tscn
  Material Codex screen/panel.
...
```text
GameState.gd
  Owns scenario state, selected object ID, current phase, and global accessors.

SimClock.gd
  Handles pause/play, speed, hourly ticks, day/hour display, and tick events.

ProjectSystem.gd
  Advances time-based projects: survey, claim, mine setup, testing, research, forging.

StockpileSystem.gd
  Manages local stockpiles and transfer rules.

LogisticsSystem.gd
  Moves shipments along routes and delivers cargo to local stockpiles.

CodexSystem.gd
  Tracks discovered material knowledge and updates Codex entries.

EventBus.gd
  Sends notifications to UI panels and event log.

DebugTools.gd
  Provides test actions: advance time, complete project, add supplies, reveal material, reset.
```

Keep these systems small. They should be readable and replaceable. If a script grows too large, split it only when it becomes painful, not preemptively.

#### Data Format Recommendation

Use simple local data first. Avoid databases, networking, accounts, cloud saves, or external services.

Recommended data files:

```text
scenario_phase1a.json
  Regions, routes, starting Hearthmere values, prospect site definitions, initial projects.

materials_redglass_variants.json
  Prototype material variants and discoverable properties.

projects.json
  Project definitions: survey, claim deposit, establish mine, test material, analyze sample, forge output.

ui_text.json
  Optional text strings for notifications, Codex entries, and summaries.
```

For the very first build, hardcoding this data in GDScript is acceptable. Move it to JSON only when the hardcoded version becomes annoying to tune.

Prototype rule:

> Start with hardcoded data if that gets the loop working faster. Move to data files when tuning becomes the bottleneck.

#### Phase 1A Code Modules

The first build only needs these conceptual modules:

1. **Scenario Loader**
   Creates Hearthmere, regions, routes, prospect sites, starting supplies, labor teams, and material variants.

2. **Clock / Tick System**
   Advances one in-game hour at a time and notifies other systems.

3. **Selection / Inspection System**
   Lets the player click map objects and see relevant data/actions.

4. **Economy System**
   Tracks Hearthmere population, workforce, labor teams, supplies, and local stockpiles.

5. **Project System**
   Handles any action that takes time: surveying, claiming, mine setup, testing, analysis, forging.

6. **Prospect / Deposit / Mine System**
   Transitions sites from prospect to confirmed deposit to active mine.

7. **Stockpile System**
   Keeps goods local. Hearthmere supplies are not the same as mine supplies. Mine ore is not usable until delivered.

8. **Shipment System**
   Moves supplies and ore between local stockpiles through visible route movement.

9. **Material / Codex System**
   Tracks hidden material profile, known properties, testing results, and Codex entries.

10. **Summary System**
    Ends Phase 1A when the player completes the first meaningful material output.

#### What Not to Build in the Tech Stack Yet

Do not build these in the first implementation pass:

- save/load system beyond maybe a reset button,
- procedural generation,
- random map generation,
- full material generation,
- raider AI,
- combat,
- squads,
- pathfinding beyond route progress,
...
- localization,
- ECS architecture,
- inventory system beyond local stockpiles,
- database layer,
- or a complex UI framework.

A boring local prototype is the goal.

#### Suggested Folder Structure

A simple Godot folder structure:
...
res://
  scenes/
    ...
    map/MapView.tscn
    ui/TopBar.tscn
    ui/InspectorPanel.tscn
    ui/CodexPanel.tscn
    ui/EventLogPanel.tscn
    ui/SummaryPanel.tscn

  scripts/
    autoload/GameState.gd
    autoload/SimClock.gd
    autoload/ProjectSystem.gd
    autoload/StockpileSystem.gd
    autoload/LogisticsSystem.gd
    autoload/CodexSystem.gd
    autoload/EventBus.gd
    autoload/DebugTools.gd
    map/MapObject.gd
    ui/InspectorPanel.gd
    ui/CodexPanel.gd
...
This is intentionally modest. It should feel like a prototype project, not a production codebase.

#### First Implementation Order in Godot

Build in this order:

1. Create Godot project.
2. Create Main scene with a blank map area and UI shell.
3. Add SimClock with pause/play and advance-hour debug button.
4. Add hardcoded regions and routes as clickable map labels/icons.
5. Add InspectorPanel showing selected object details.
6. Add Hearthmere state: population, workforce, labor teams, supplies.
7. Add generic ProjectSystem.
8. Add three prospect sites and survey project.
9. Add one true Redglass result and first Codex entry.
10. Add claim deposit and establish mine project.
11. Add local stockpiles at Hearthmere and mine.
12. Add supply shipment from Hearthmere to mine.
13. Add ore production at supplied mine.
14. Add ore shipment from mine to Hearthmere.
15. Add material test / academy analysis project.
16. Add forge output project.
17. Add Phase 1A summary screen.
18. Add debug tools for tuning.

Stop there. Do not add raiders until this loop works.

#### Recommended First Prototype UI Layout

Use a plain three-zone layout:

```text
+------------------------------------------------------+
| Top Bar: Day/Hour | Pause/Play | Speed | Debug       |
+----------------------------+-------------------------+
|                            |                         |
|         Map View           |   Inspector Panel        |
|                            |                         |
|  Regions / Routes / Sites  |   Selected object info   |
|                            |   Context actions        |
|                            |                         |
+----------------------------+-------------------------+
...
Add a Codex button/panel when material discovery exists. Do not overdesign the UI. It only needs to be readable enough to test decisions.
...
- **VS Code** if preferred for editing scripts and JSON.
- **Git** for version control.
- A simple `notes/` or `design-log.md` file for playtest notes.
- Screenshots or short screen recordings for each milestone.

Recommended repository rule:

> Commit every time a milestone works, even if the UI is ugly.

Suggested commit names:

```text
init godot project shell
add clickable phase1 map
add hearthmere economy state
add generic project system
add prospect survey loop
add material codex entry
add mine setup and local stockpiles
add shipment movement
add material testing and forge output
add phase1a summary screen
```

#### Solo Developer Stack Principle

> The best stack is the one that lets one person build a complete, ugly, testable loop before enthusiasm runs out.

For AXIOM, that means Godot 4.x stable, GDScript, 2D, desktop, local data, simple panels, hardcoded scenario data first, and no raiders until the economy/discovery/logistics spine works.
